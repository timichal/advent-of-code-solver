export const fifteenEighteen = (input: string[]): [number, number] => {
  ...
  */
  ...
  const litNeighbors = (grid: string[][], x: number, y: number) => {
    let lit = 0;
    if (grid[x - 1]) {
  ...
  const gridStep = (grid: string[][], cornerLightStuck?: boolean) => {
    const oldGrid = grid;
  ...
  const part1 = JSON.stringify(part1Grid).match(/#/g)?.length ?? 0;
  const part2 = JSON.stringify(part2Grid).match(/#/g)?.length ?? 0;
