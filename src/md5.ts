...
// the file is base64'd compiled wasm implementation of md5 from here: https://github.com/biggun1860/md5-sha1-wasm
...
// String transforms
const UTF8ToString = (idx: number) => {
  let endPtr = idx;
  while (HEAPU8[endPtr]) endPtr += 1;
  return new TextDecoder("utf8").decode(HEAPU8.subarray(idx, endPtr));
};

const stringToUTF8 = (str: string, outIdx: number, maxBytesToWrite: number) => {
...
// as of TypeScript 3.9.6, the typings for WebAssembly errors are wrong, so we throw a general error
const abort = (what: string) => { throw new Error(`WebAssembly error: ${what}`); };
...
const asmLibraryArg = {
  ...
  emscripten_memcpy_big: (dest: number, src: number, num: number) => HEAPU8.set(HEAPU8.subarray(src, src + num), dest),
...
WebAssembly
  .instantiateStreaming(fetch(file), { env: asmLibraryArg })
  .then((result) => { wasmInstance = result.instance.exports; });

export const md5 = (input: string) => {
  const toCstring = (str: string) => {
    let ret = 0;
    if (str !== null && str !== undefined) { // null string
      // at most 4 bytes per UTF-8 code point, +1 for the trailing '\0'
      const len = (str.length << 2) + 1;
      ret = wasmInstance.stackAlloc(len);
      stringToUTF8(str, ret, len);
    }
    return ret;
  };

  const stack = wasmInstance.stackSave();
  const ret = wasmInstance.md5_hex(toCstring(input));
